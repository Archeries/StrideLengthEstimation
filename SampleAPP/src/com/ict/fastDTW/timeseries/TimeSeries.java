...
package com.ict.fastDTW.timeseries;

import com.ict.fastDTW.util.Arrays;
...
public class TimeSeries {
	...
	public TimeSeries(double[][] data)   {
		this(data, ZERO_ARRAY, DEFAULT_IS_TIME_1ST_COL,
				DEFAULT_IS_LABELED, DEFAULT_DELIMITER);
	}

	public TimeSeries(double[][] data, int[] colToInclude,
			boolean isFirstColTime, boolean isLabeled, char delimiter)  {
		this();

 
		try {
			String line=java.util.Arrays.toString(data[0]);
			StringTokenizer st = new StringTokenizer(line,
					String.valueOf(delimiter));
...
//				br = new BufferedReader(new FileReader(inputFile)); // open the
			...
			for (int i = 0; i < data.length; i++) {
				...
				for (int j = 0; j < data[i].length; j++) {
					currentLineValues.add(data[i][j]);
				}
				if (isFirstColTime)
					timeReadings.add(currentLineValues.get(0));
				else
	...
	public TimeSeries(String filepath) {
		tsArray=gettsArray(filepath);
		labels=getlabels(filepath);
		timeReadings=gettimeReadings(filepath);
		
		
	}
	
	public TimeSeries(ArrayList tsArray,ArrayList labels,ArrayList timeReadings) {
	...
	public static ArrayList getlabels(String filepath) {
...
//		String line = br.readLine(); // the top row that contains attribiute
				...
				while (st1.hasMoreTokens()) {
					final String currentToken = st1.nextToken();
					labels.add(new String("c" + c)); // TODO change to
					c++;
				}
			} catch (FileNotFoundException e) {
	...
	public static ArrayList gettimeReadings(String filepath) {
		...
		int count=0;
			try {
              ...
              while ((line = br.readLine()) != null) // read lines until end of
				// file
				{
				  timeReadings.add(new Double(count));
				   count++;
				}
	    	} catch (FileNotFoundException e) {
				// TODO Auto-generated catch block
	...
	public static ArrayList gettsArray(String filepath) {
